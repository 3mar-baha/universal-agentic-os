...
cmd_install() {
    ...
    mkdir -p "$bin_dir"
    if ln -sf "${SCRIPT_DIR}/uos.sh" "$target" 2> /dev/null; then
        chmod +x "$target" 2> /dev/null || true
        echo "[install] linked $target -> ${SCRIPT_DIR}/uos.sh"
    else
        # Windows hosts without symlink privilege fall back to a copy.
        cp "${BASH_SOURCE[0]}" "$target" && chmod +x "$target" 2> /dev/null || true
        echo "[install] copied CLI to $target (symlink unavailable on this host)"
    fi
    case ":${PATH}:" in
        *":${bin_dir}:"*) ;;
